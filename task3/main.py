...
class FitnessApp:
    def __init__(self, root):
        ...
        # Title
        tk.Label(root, text="Fitness Tracker Dashboard", font=("Arial", 18, "bold"), bg="white", fg="#333").pack(pady=10)
        ...
        # Buttons
        tk.Button(root, text="Log Entry", command=self.log_entry, bg="#28a745", fg="white").pack(pady=10)
        tk.Button(root, text="Show Weekly Report", command=self.show_weekly_report, bg="#007bff", fg="white").pack(pady=5)
        tk.Button(root, text="Show Weekly Pie Chart", command=self.show_pie_chart, bg="#6c757d", fg="white").pack(pady=5)
    ...
    def show_pie_chart(self):
        try:
            ...
            total_steps = sum(row[1] for row in data if row[1])
            total_workout = sum(row[2] for row in data if row[2])
            total_calories = sum(row[3] for row in data if row[3])

            labels = ['Steps', 'Workout (min)', 'Calories']
            sizes = [total_steps, total_workout, total_calories]

            if sum(sizes) == 0:
                messagebox.showinfo("Info", "No data to display in pie chart.")
                return
